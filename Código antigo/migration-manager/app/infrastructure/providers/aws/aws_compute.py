"""Implementacao inicial de ComputeProvider para o ambiente experimental.

Esta implementacao (`DockerComputeProvider`) simula a criacao de containers
localmente, servindo como stub compativel com o ambiente Floci enquanto uma
integracao real com Docker/ECS nao e adicionada. Ela cumpre o contrato
`ComputeProvider` por completo, permitindo que o restante do sistema seja
desenvolvido e testado sem depender de infraestrutura real.
"""
...
class DockerComputeProvider(ComputeProvider):
    """Compute provider experimental compativel com o Floci (emulador AWS).

    Nao acopla o dominio a Docker/boto3 diretamente: toda a configuracao de
    endpoint (ex: `AWS_ENDPOINT_URL`) e resolvida a partir de variaveis de
    ...
    def health_check(self, deployment: Deployment) -> HealthCheckResult:
        ...
        return HealthCheckResult(status=status, detail="stub health check (Floci)")
