...
Floci AWS e Gateway (`4566` e `8080`) já são do projeto microservices-demo
e não são tocados por aqui. Suba o Floci-AZ pela sua própria fonte — seu
compose/projeto de origem, não o deste repositório — e depois o manager,
que sobe sozinho pelo compose deste projeto:

```bash
docker compose up -d --build migration-manager
```

O endpoint de cada lado (`http://host.docker.internal:4566` para o Floci
AWS, o endereço do Floci-AZ) vai no `endpoint` de `source`/`target` dentro
do JSON da migração — o manager não lê nenhuma variável de ambiente para
isso.
